# IAM Roles and Policies for Syncertica Enterprise
...
# Custom ECS Execution Policy for Secrets Manager
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = var.enable_encryption ? aws_kms_key.secrets[0].arn : "*"
      }
    ]
  })
}

# ECS Task Policy for Application
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:anandavicky123/syncerticaenterprise:*"
          }
        }
      }
    ]
...
resource "aws_kms_key" "secrets" {
  ...
  description             = "KMS key for secrets encryption in ${var.project_name}-${var.environment}"
