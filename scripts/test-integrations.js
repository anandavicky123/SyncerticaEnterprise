...
function testAWSCredentials() {
  ...
  try {
    ...
    const awsVersion = execSync("aws --version", {
      encoding: "utf8",
      stdio: "pipe",
    });
...
function testDocker() {
  ...
  try {
    ...
    const dockerInfo = execSync("docker info", {
      encoding: "utf8",
      stdio: "pipe",
    });
    ...
    const buildTest = execSync("docker build --help", {
      encoding: "utf8",
      stdio: "pipe",
    });
...
function testApplicationDeps() {
  ...
  try {
    ...
    const packageCheck = execSync("npm list --depth=0", {
      encoding: "utf8",
      stdio: "pipe",
    });
...
async function runAllTests() {
  ...
  if (passedTests === totalTests) {
    ...
    console.log(
      "⚠️  Some tests failed - Check configuration before deployment"
    );
  }
}
