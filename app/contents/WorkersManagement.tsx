...
const WorkersManagement: FC<WorkersManagementProps> = ({
  className = "",
}) => {
  ...
  const fetchWorkers = useCallback(async () => {
    try {
      ...
      if (error instanceof SyntaxError) {
        // JSON parse error - likely got HTML instead of JSON
        window.location.href = "/login";
        return;
      }
      const message =
  ...
  }, [setWorkers, setLoading, setErrorMessage, showToast]);
  ...
  if (editingWorker) {
        ...
        if (!response.ok) {
          ...
          // Show error message
          const errorMsg = data.error || "Failed to update worker";
