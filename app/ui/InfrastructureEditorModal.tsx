...
import { X, Save, FileCog, ChevronDown } from "lucide-react";
...
const InfrastructureEditorModal: React.FC<Props> = ({
  isOpen,
  onClose,
  item,
  onSaved,
...
# This is a template for infrastructure configuration
...
`,
  ...
  useEffect(() => {
    if (item) {
      ...
      // Reset for new item
      setContent(`# Infrastructure Configuration
# This is a template for infrastructure configuration
# Replace this with your actual infrastructure code
...
`);
  ...
  useEffect(() => {
    const handleGitHubConnection = () => {
      console.log(
        "GitHub connection established in InfrastructureEditorModal, refreshing repositories...",
      );
      refetchRepositories();
    };
    ...
    try {
      ...
      const res = await fetch("/api/github/contents", {
        ...
        body: JSON.stringify({
          ...
          message: item
            ? `chore(infra): update ${finalPath}`
            : `feat(infra): add ${finalPath}`,
        }),
      });
      if (!res.ok) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
          <div className="flex items-center gap-3">
            <FileCog className="w-6 h-6 text-orange-500" />
            <h2 className="text-lg font-semibold text-gray-900">
              {isCreate ? "Create Infrastructure" : "Edit Infrastructure"}
            </h2>
          </div>
                ...
                {reposLoading ? (
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent appearance-none pr-10 bg-white"
                    >
        ...
        <div className="flex-1 p-6 overflow-hidden">
          ...
          <textarea
            ...
            placeholder="Enter your infrastructure configuration (Terraform, CloudFormation, etc.)"
          />
        </div>
      </div>
    </div>
